...
public class AddressController(IAddressServices addressServices) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateAddressAsync([FromHeader, Required] string userId, [FromBody, Required] Address address)
    {
        var response = await _addressServices.UpdateAddressAsync(userId, address);
    ...
    [ProducesResponseType(typeof(ApiResponse<string>), StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> DeleteAddressAsync([FromHeader, Required] string userId, [FromHeader, Required] string addressId)
    {
